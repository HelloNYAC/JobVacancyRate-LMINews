  ...
  getDataFromVectorByReferencePeriodRange(
    ...
    function (res) {
      if (coordinate.endsWith("4")) {
        avgRCountryArray = res[0].object.vectorDataPoint.map(function (d) {
          return {
            refPer: d.refPer,
            value: d.value
          };
        ...
        localStorage.setItem(
          "avgRCountryArray",
          JSON.stringify(avgRCountryArray)
        );
      } else {
        avgCountryArray = res[0].object.vectorDataPoint.map(function (d) {
          return {
            refPer: d.refPer,
            value: d.value
          };
        ...
        localStorage.setItem(
          "avgCountryArray",
          JSON.stringify(avgCountryArray)
        );
...
function updateProvinceAvg(coordinate) {
  ...
  getDataFromVectorByReferencePeriodRange(
    ...
    function (res) {
      if (coordinate.endsWith("4")) {
        var updateRProvinceAvg = res[0].object.vectorDataPoint.map(function (
          d
        ) {
          return {
            refPer: d.refPer,
        ...
        localStorage.setItem(
          "updateRProvinceAvg",
          JSON.stringify(updateRProvinceAvg)
        );
      } else {
        var updateProvinceAvg = res[0].object.vectorDataPoint.map(function (d) {
          return {
            refPer: d.refPer,
            value: d.value
          };
        ...
        localStorage.setItem(
          "updateProvinceAvg",
          JSON.stringify(updateProvinceAvg)
        );
      } // console.log(JSON.parse(localStorage.getItem("avgRCountryArray")))
    }
  );
...
$(function () {
  ...
  var productId = 14100326;
  var economic_region = data.economic_region;
  var statisticsVal = statistics[data.statistics];
  var coordinate = ""
    .concat(data.province, ".")
    .concat(data.naics, ".")
    .concat(data.statistics, ".0.0.0.0.0.0.0");

  if (data.province == 1 || data.naics == 1) {
    coordinate = ""
      .concat(data.province, ".")
      .concat(data.naics, ".")
      .concat(data.statistics);
  }

  coordinateFlag = "1.1.".concat(data.statistics);
  updateCountryAvg("1.1.".concat(data.statistics));
  updateProvinceAvg("".concat(data.province, ".1.").concat(data.statistics));
  ...
  console.log(
    "vectorID= "
      .concat(vectorID, " , statistics= ")
      .concat(statisticsVal, " , coordinate=")
      .concat(coordinate, "  , geo= ")
      .concat(geo, "  ,  naicsVal= ")
      .concat(naicsVal)
  );
  ...
  getDataFromVectorByReferencePeriodRange(
    ...
    function () {
      $("#msgDiv").html(
        "There is no data available to display for your search"
      );
  ...
  if (ecMap[economic_region]) {
    paramStr += "&economic_region=*".concat(ecMap[economic_region], "*");
  }

  if (naicsMap[naicsVal]) {
    paramStr += "&naics=*".concat(naicsMap[naicsVal], "*");
  }

  if (geo) {
    paramStr += "&province=*".concat(ptLabeltoProvCode(geo, ""), "*");
  }
    ...
    if (res.data && res.data.length > 0) {
      ...
      getNews(paramStr, function (res) {
        ...
        if (res.data && res.data.length > 0) {
          ...
          $(".newsDiv").html(
            "There is no data available according to your search criterias. Please try again."
          );
        }
      });
    }
...
function getVectorIds(data) {
  ...
  var pattern = new RegExp(
    "".concat(province, ".").concat(naics, ".").concat(data.statistics)
  );
  var vectorIDs = [];
...
function renderNews(data) {
  var template =
    '<div style="padding:10px" class="new shadow-blur" id="news"><a href="_href" target="blank">' +
    "<h3> _title_fr</h3>" + // '<p>link_fr:_link_fr</p>' +
  ...
  data.forEach(function (d) {
    var news = template
      .replace("_title_fr", d.title_en || d.title_fr)
...
function renderTable(data) {
  data = data || [];
  var avgRCountryArray = JSON.parse(
    localStorage.getItem("avgRCountryArray") || "[]"
  );
  var avgCountryArray = JSON.parse(
    localStorage.getItem("avgCountryArray") || "[]"
  );
  var avgArray = coordinateFlag.endsWith("4")
    ? avgRCountryArray
    : avgCountryArray;
  var updateRProvinceAvg = JSON.parse(
    localStorage.getItem("updateRProvinceAvg") || "[]"
  );
  var updateProvinceAvg = JSON.parse(
    localStorage.getItem("updateProvinceAvg") || "[]"
  );
  var avgArray2 = coordinateFlag.endsWith("4")
    ? updateRProvinceAvg
    : updateProvinceAvg;
  $("#jvrbyindustry")
      ...
      columns: [
        ...
        },
        {
          ...
          formatter: function formatter(value, row, index) {
            var tt = frequency.find(function (s) {
              return +s.frequencyCode == +value;
            });
        ...
        },
        {
          ...
          width: "200px",
          formatter: function formatter(value, row, index) {
            var tt = avgArray2.find(function (s) {
              return s.refPer == row.refPer;
            });
        ...
        },
        {
          ...
          width: "200px",
          formatter: function formatter(value, row, index) {
            var tt = avgArray.find(function (s) {
              return s.refPer == row.refPer;
            });
            if (tt) return tt.value;
            return value;
...
function getDataFromVectorByReferencePeriodRange(
  vectorId,
  startRefPeriod,
  endReferencePeriod,
  cbFunction,
  ...
  var url =
    "https://www150.statcan.gc.ca/t1/wds/rest/getDataFromVectorByReferencePeriodRange?vectorIds="
      .concat(vectorId, "&startRefPeriod=")
      .concat(startRefPeriod, "&endReferencePeriod=")
      .concat(endReferencePeriod);
  $.ajax({
    url: url,
    type: "GET",
    dataType: "json",
...
function getBulkVectorDataByRange(
  vectorIds,
  ...
  var url = "https://www150.statcan.gc.ca/t1/wds/rest/getBulkVectorDataByRange";
  $.ajax({
    url: url,
    method: "POST",
