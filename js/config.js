...
var ptCodeArray = return_PTcodes_array();
var erCodeArray = return_ECcodes_array();
var naicsArray = return_NAICSCodes_array();
...
$(document).on({
  "lity:ready": function lityReady(event, lightbox) {
    $("body").css("overflow", "hidden");

    if (event.target.firstChild.localName == "iframe") {
      jQuery(event.currentTarget.activeElement)
        .find(".lity-iframe-container > iframe")
        .attr("title", lightbox.opener().data("title"));
    }
  },
  "lity:close": function lityClose(e) {
  ...
  if (typeof instance.opener()[0] != "undefined") {
    if (typeof instance.opener()[0].dataset.section != "undefined") {
      if (pageTopic == "occupations") {
        ...
        var oldID = instance.opener()[0].dataset.section;
        var oldIDArray = instance.opener()[0].dataset.section.split("-");
        var newID = oldIDArray[0] + "-" + oldIDArray[1];
